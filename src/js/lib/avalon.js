...
/***/ function(module, exports, __webpack_require__) {
	...
	//验证单个表单元素
	    ...
	    validateAll: function (callback) {
	        var validator = this
			var vmodel;
			if(validator.fields&&validator.fields.length>0){
				vmodel = validator.fields[0].vmodel
			}
	        ...
	        return Promise.all(promise).then(function (array) {
	            ...
	            fn.call(validator.dom, reasons,vmodel) //pw
	        })
	    },
	    ...
	    validate: function (field, isValidateAll, event) {
	        ...
	        return Promise.all(promises).then(function (array) {
	            ...
	            if (!isValidateAll) {
	                if (reasons.length) {
	                    validator.onError.call(elem, reasons,vmodel, event)
	                } else {
	                    validator.onSuccess.call(elem, reasons,vmodel, event)
	                }
	                validator.onComplete.call(elem, reasons,vmodel, event)
	            }
	            return reasons
	        })
	    }
