...
(function (RongIMLib) {
    var RongIMClient = (function () {
        ...
        RongIMClient.init = function (appKey, dataAccessProvider, options) {
            ...
            var opts = RongIMLib.MessageUtil.buildOptions(options, {
        ...
        RongIMClient.prototype.sendMessage = function (conversationType, targetId, messageContent, sendCallback, mentiondMsg, pushText, appData) {
            ...
            RongIMClient.bridge.pubMsg(conversationType.valueOf(), content, targetId, {
                onSuccess: function (data) {
                    data && data.timestamp && RongIMClient._storageProvider.setItem('converST_' + RongIMLib.Bridge._client.userId + conversationType + targetId, data.timestamp);
        ...
        RongIMClient.prototype.sendReceiptResponse = function (conversationType, targetId, sendCallback) {
            ...
            if (RongIMLib.MessageUtil.supportLargeStorage()) {
                var valObj = JSON.parse(RongIMClient._storageProvider.getItem(rspkey));
                if (valObj) {
                    var vals = [];
                ...
                onSuccess: function (data) {
                    ...
                    if (RongIMLib.MessageUtil.supportLargeStorage()) {
                        for (var i = 0, len = list.length; i < len; i++) {
                            ...
                            tempDir = JSON.parse(RongIMClient._storageProvider.getItem(RongIMLib.Bridge._client.userId + tempMsg.messageUId + "SENT"));
                            if (tempDir) {
                ...
                onSuccess: function () {
                    ...
                    RongIMLib.Bridge._client.queryMessage("chrmPull", RongIMLib.MessageUtil.ArrayForm(modules.toArrayBuffer()), chatroomId, 1, {
                        onSuccess: function (collection) {
                            ...
                            RongIMClient._storageProvider.setItem(chatroomId + RongIMLib.Bridge._client.userId + "CST", sync);
...
(function (RongIMLib) {
    ...
    var MessageUtil = (function () {
        ...
        MessageUtil.buildOptions = function (one, opts, protocol) {
